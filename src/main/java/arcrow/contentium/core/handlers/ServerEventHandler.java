...
import arcrow.contentium.ModEnchantments;
...
import arcrow.contentium.entities.ai.EntityAIOpenCustomDoor;
...
public class ServerEventHandler {
	...
	public void harvestEvent(BlockEvent.HarvestDropsEvent event) {
		if (Contentium.enableSilkTouchingMushrooms && event.isSilkTouching)
		...
		if (Contentium.enableSticksFromDeadBushes)
		...
		if (Contentium.enableShearableCobwebs)
	...
	public void dropEvent(LivingDropsEvent event) {
		...
		if (Contentium.enableSkullDrop)
			dropHead(event.entityLiving, event.source, event.lootingLevel, event.drops);

		Random rand = event.entityLiving.worldObj.rand;
		if (Contentium.enableMutton && event.entityLiving instanceof EntitySheep) {
	...
	public void interactEntityEvent(EntityInteractEvent event) {
		...
		if (!animal.isChild()) {
			if (animal instanceof EntityPig) {
				if (stack.getItem() == ModItems.beetroot && Contentium.enableBeetroot)
					setAnimalInLove(animal, event.entityPlayer, stack);
			} else if (animal instanceof EntityChicken)
				if (stack.getItem() == ModItems.beetroot_seeds && Contentium.enableBeetroot)
					setAnimalInLove(animal, event.entityPlayer, stack);
		} else if (Contentium.enableBabyGrowthBoost && isFoodItem(animal, stack))
	...
	private boolean isFoodItem(EntityAnimal animal, ItemStack food) {
		if (animal.isBreedingItem(food))
			return true;
		else if (animal instanceof EntityPig && food.getItem() == ModItems.beetroot && Contentium.enableBeetroot)
			return true;
		else if (animal instanceof EntityChicken && food.getItem() == ModItems.beetroot_seeds && Contentium.enableBeetroot)
			return true;
		else
			return false;
	}
	...
	public void entityHurtEvent(LivingHurtEvent event) {
		if (!Contentium.enableDmgIndicator)
			return;
		...
		if (event.source instanceof EntityDamageSource) {
			...
			if (attacker instanceof EntityPlayer && !(attacker instanceof FakePlayer)) {
				...
				for (int i = 0; i < amount; i++) {
					...
					Contentium.networkWrapper.sendToAllAround(new BlackHeartParticlesMessage(x, y, z), new TargetPoint(player.worldObj.provider.dimensionId, x, y, z, 64));
				}
			}
		}
	}
	...
	public void entityStruckByLightning(EntityStruckByLightningEvent event) {
		if (Contentium.enableVillagerTurnsIntoWitch && event.entity instanceof EntityVillager) {
			EntityVillager villager = (EntityVillager) event.entity;
